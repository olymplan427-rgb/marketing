...
-- =====================================================
-- 1) 메인 스키마 (content_history, saved_prompt_templates, user_settings)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS saved_prompt_templates (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id TEXT NOT NULL,
    template_name TEXT NOT NULL,
    template_type TEXT NOT NULL CHECK (template_type IN ('topic', 'field', 'blog_post')),
    content TEXT NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

CREATE INDEX IF NOT EXISTS idx_prompt_templates_user_id ON saved_prompt_templates(user_id);
CREATE INDEX IF NOT EXISTS idx_prompt_templates_type ON saved_prompt_templates(template_type);
...
CREATE TRIGGER update_prompt_templates_updated_at
    BEFORE UPDATE ON saved_prompt_templates
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
ALTER TABLE saved_prompt_templates ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Users can view their own templates" ON saved_prompt_templates FOR SELECT
    USING (auth.uid()::text = user_id OR user_id LIKE 'browser_%');
CREATE POLICY "Users can insert their own templates" ON saved_prompt_templates FOR INSERT
    WITH CHECK (auth.uid()::text = user_id OR user_id LIKE 'browser_%');
CREATE POLICY "Users can update their own templates" ON saved_prompt_templates FOR UPDATE
    USING (auth.uid()::text = user_id OR user_id LIKE 'browser_%');
CREATE POLICY "Users can delete their own templates" ON saved_prompt_templates FOR DELETE
    USING (auth.uid()::text = user_id OR user_id LIKE 'browser_%');
...
-- =====================================================
SELECT '✅ Supabase 통합 셋업 완료 (테이블 6개 + 스토리지 버킷 2개)' AS result;
